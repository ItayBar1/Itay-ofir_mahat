...
interface CourseCardProps {
  course: any; // אפשר להגדיר טיפוס מדויק יותר בהמשך
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-all flex flex-col h-full">
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className="text-lg font-bold text-slate-800">₪{course.price_ils}</span>
        </div>
        ...
        <div className="space-y-2 text-sm text-slate-600 mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>יום {DAY_MAP[course.day_of_week]} • {course.start_time.substring(0, 5)} - {course.end_time.substring(0, 5)}</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        >
          {isFull ? 'הרשמה נסגרה' : 'הרשמה ותשלום'}
        </button>
      </div>
    </div>
  );
