...
export const InstructorSchedule: React.FC = () => {
      ...
      try {
        ...
        if (data) {
          const formatClassForDisplay = (cls: any) => {
            ...
            const startStr = cls.start_time || "00:00";
            const endStr = cls.end_time || "00:00";
  ...
  const displayedClasses = selectedDay === 'all' 
    ? classes 
    ...
    <div className="space-y-6">
      <div className="flex flex-col space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">מערכת השעות שלי</h2>
            <p className="text-slate-500 mt-1">
              {selectedDay === 'all' 
                ? `מציג את כל ${classes.length} השיעורים שלך`
                : `מציג ${displayedClasses.length} שיעורים ליום ${days[selectedDay as number]}`
              }
            </p>
          </div>
      ...
      <div className="border-t border-slate-200 pt-6">
        {displayedClasses.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900">לא נמצאו שיעורים</h3>
